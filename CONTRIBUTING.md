...
audit executed through the installed wheel on 3.12. CI passed 8/8 jobs for the 0.1.0 publishing
workflow commit on 2026-09-23; future commits must pass their own run. The 48 000-file scale test
is skipped by every one of those runs (`tests/test_scale.py` enables it only when
`DATASET_DOCTOR_SCALE=1`) because its
