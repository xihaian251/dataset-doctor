...
def _tabular_config() -> str:
    return """# Generated by `dataset-doctor-audit demo`. Declaring these fields is what lets the
...
Every row of this table was read back out of an actual `dataset-doctor-audit audit` run, so the
...
def _image_config() -> str:
    return """# Generated by `dataset-doctor-audit demo`.
dataset:
  type: image
labels:
  source: directory
...
Every row below was read back out of an actual `dataset-doctor-audit audit` run, with the
