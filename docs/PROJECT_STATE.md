...
| Area | State |
| --- | --- |
| Package | `dataset-doctor-audit` 0.1.2 in this tree (published index still serves 0.1.1 until this release goes out; importable `dataset_doctor_audit`, CLI `dataset-doctor-audit`) |
...
| Next release | 0.1.1 shipped 2026-09-25: defects 5-8 released as a real-world validation patch (no new rule, no changed severity default, no changed verdict algorithm). `pyproject.toml` is the version source of truth for the artefact and `__version__` the one the reports carry; `tests/test_packaging.py` fails if the two disagree. **0.1.2 prepared 2026-09-25** with exactly one scope: section 4 defect 9 (DD013 answering `PASS` for image folders), fixed as `117d161`. Gate re-measured on that tree: `pytest` 234 passed / 30 skipped in 67.03 s · `ruff format --check .` 107 files already formatted · `ruff check .` clean · `mypy --python-version=3.13 dataset_doctor_audit` 32 files clean. The two P2 counters in section 10 item 11 and the relative-path contract in section 10 item 9 stay registered and unfixed, and with this patch the real-world acceptance programme closes |
