...
Last updated: 2026-09-23. The older measured figures below retain their original dates and
environment (the authoring machine, Windows 11 / Python 3.13.1) unless stated otherwise.
...
| Area | State |
| --- | --- |
| Package | `dataset-doctor-audit` 0.1.2, the version PyPI serves as latest (`pyproject.toml` and `dataset_doctor_audit.__version__` both 0.1.2; importable `dataset_doctor_audit`, CLI `dataset-doctor-audit`) |
...
| Published 0.1.2 | [PyPI](https://pypi.org/project/dataset-doctor-audit/0.1.2/) carries wheel `3d7c73572d09985715de8af51dbe9e45c266a1dcee3829f823fc89dc084a9f1d` (146 928 B) and sdist `d785c10a2f5f6f04d57d64a0d955e4c2afe386a81133517029900342a8f82ecb` (440 442 B), uploaded 2026-09-25T16:01Z. [TestPyPI run 36156039925](https://github.com/xihaian251/dataset-doctor/actions/runs/36156039925) built `107504e` on Linux and produced those bytes first; [production run 36157278939](https://github.com/xihaian251/dataset-doctor/actions/runs/36157278939) rebuilt frozen commit `107504ed7d2f0caffea9dbc28ec25225a532fdb0`, matched both hashes under `sha256sum --check --strict`, and published through Trusted Publishing with `pypi` environment approval - its publish step emitted in-toto `docs.pypi.org/attestations/publish/v1` statements naming exactly those two digests. Annotated `v0.1.2` points at `107504e`. Unlike 0.1.0/0.1.1 the [GitHub Release](https://github.com/xihaian251/dataset-doctor/releases/tag/v0.1.2) carries **no binary assets** (reason and measurement in `docs/RELEASE_CHECKLIST.md` section 4c), so the byte-equivalence chain for this version runs artifact → workflow pin → PyPI. A fresh non-system environment installed `dataset-doctor-audit[image]==0.1.2` from PyPI and reproduced the DD013 image fix |
| Next release | 0.1.1 shipped 2026-09-25: defects 5-8 released as a real-world validation patch (no new rule, no changed severity default, no changed verdict algorithm). `pyproject.toml` is the version source of truth for the artefact and `__version__` the one the reports carry; `tests/test_packaging.py` fails if the two disagree. **0.1.2 released with exactly one scope**: section 4 defect 9 (DD013 answering `PASS` for image folders), fixed as `117d161` and published from `107504e`. Gate re-measured on that tree: `pytest` 234 passed / 30 skipped in 67.03 s · `ruff format --check .` 107 files already formatted · `ruff check .` clean · `mypy --python-version=3.13 dataset_doctor_audit` 32 files clean. The two P2 counters in section 10 item 11 and the relative-path contract in section 10 item 9 stay registered and unfixed; with this patch the real-world acceptance programme closes and the project is frozen at 0.1.2, so a fifth dataset, either P2 counter, the relative-path contract, a new rule or any GUI/API work each needs its own authorisation rather than being folded into a patch |
...
| CI and publication, 2026-09-23 | [CI run 35821169961](https://github.com/xihaian251/dataset-doctor/actions/runs/35821169961) passed 8/8 jobs; [production run 35858748946](https://github.com/xihaian251/dataset-doctor/actions/runs/35858748946) succeeded | GitHub Actions API and PyPI 0.1.0 JSON/provenance; the 2026-09-22 zero-run observation is superseded |
...
3. ~~Real repository~~ Done 2026-09-22: `git init -b main` and one root commit carrying all 481
   ...
   **Completed 2026-09-23**: the first push, private vulnerability reporting, the 0.1.0 PyPI
   publication, annotated `v0.1.0` tag at the frozen source commit, and GitHub Release. The
   published 0.1.0 README still has its pre-release sentence; the correction on `main` is for
   the next release, not a replacement of an immutable PyPI file.
4. ~~Release engineering artefacts~~ Written 2026-09-22, then executed 2026-09-23: `.github/workflows/ci.yml`
   ...
   `ABOUT.md`), `.github/PULL_REQUEST_TEMPLATE.md` and `docs/RELEASE_CHECKLIST.md`. The first
   push and subsequent runs established the CI and Trusted Publishing outcomes recorded above.
