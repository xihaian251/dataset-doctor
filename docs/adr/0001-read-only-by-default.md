...
`dataset_doctor_audit` has no code path that writes to a dataset. Adapters open files read-only; the
...
- Users who want an automated cleaner must use another tool (the PyPI `dataset-doctor` that owned our
  short name is exactly that - `docs/COMPETITIVE_ANALYSIS.md`, and ADR 0006 for the namespace split).
  That confusion is a marketing cost we accept deliberately.
